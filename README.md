...
The dashboard supports `↑`/`↓` or `j`/`k` to select, `r` to refresh, `c` to switch between cards and compact rows, `e` to show or hide emails, `s` for settings, `?` for help, and `q` or `Esc` to quit. The settings panel works in both dashboards: `↑`/`↓` to move, `space` to toggle, `←`/`→` to change the refresh interval, `esc` to go back. Each change is written to the config file right away and applies now, except `mini by default`, which applies on the next launch. When output is piped, `--mini` prints once even with live mode enabled. `--print` and `--json` always exit after one fetch.
